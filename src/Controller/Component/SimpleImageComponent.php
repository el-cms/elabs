...
class SimpleImageComponent extends Component{
	...
	function load($filename) {
		...
		// Logging this
		$this->siLog('Image loaded (' . $filename . ')');
		return true;
	}
	/**
	 * Saves an image
	...
	function save($filename, $original = false, $image_type = null, $compression = 75, $permissions = null) {
		...
		$this->siLog((($original === true) ? 'Original' : 'Copy') . ' image saved successfully.');
		return true;
	}
	/**
	...
	function resizeToHeight($height) {
		...
		$this->siLog('Image resized to height ' . $height);
	}
	/**
	...
	function resizeToWidth($width) {
		...
		$this->resize($width, $height);
		$this->siLog('Image resized to width ' . $width);
	}
	/**
	...
	function crop($width, $height, $startX = 0, $startY = 0) {
		...
		$this->currentImage = $new_image;
		$this->siLog("Crop: X=$width, Y=$height");
	}
	/**
	...
	function centerCrop($width, $height) {
		...
		$this->siLog("Center crop: W=$width, H=$height, SX=$startX, SY=$startY");
	...
	function waterMark($source, $position = 'bottom-left') {
		...
		//switch ($type) {
			...
			//case 'extend':
				//break;
		//}
		$this->siLog("Image watermarked with file $source");
	}
