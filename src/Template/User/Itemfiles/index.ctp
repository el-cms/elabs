<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('New Itemfile'), ['action' => 'add']) ?></li>
        <li><?= $this->Html->link(__('List Files'), ['controller' => 'Files', 'action' => 'index']) ?></li>
        <li><?= $this->Html->link(__('New File'), ['controller' => 'Files', 'action' => 'add']) ?></li>
    </ul>
</nav>
<div class="itemfiles index large-9 medium-8 columns content">
    <h3><?= __('Itemfiles') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th><?= $this->Paginator->sort('id') ?></th>
                <th><?= $this->Paginator->sort('model') ?></th>
                <th><?= $this->Paginator->sort('fkid') ?></th>
                <th><?= $this->Paginator->sort('file_id') ?></th>
                <th class="actions"><?= __('Actions') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($itemfiles as $itemfile): ?>
            <tr>
                <td><?= $this->Number->format($itemfile->id) ?></td>
                <td><?= h($itemfile->model) ?></td>
                <td><?= $this->Number->format($itemfile->fkid) ?></td>
                <td><?= $itemfile->has('file') ? $this->Html->link($itemfile->file->name, ['controller' => 'Files', 'action' => 'view', $itemfile->file->id]) : '' ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $itemfile->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $itemfile->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $itemfile->id], ['confirm' => __('Are you sure you want to delete # {0}?', $itemfile->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
        </ul>
        <p><?= $this->Paginator->counter() ?></p>
    </div>
</div>
